import java.io.*;

public class Main {
    static int N;

    static void _bar(String str, int stk, BufferedWriter bw) throws IOException {
        for (int j = 0; j < stk; ++j) bw.write("____");
        bw.write(str);
    }

    static void solve(int cnt, BufferedWriter bw) throws IOException {
        _bar("\"재귀함수가 뭔가요?\"\n", cnt, bw);
        if (cnt == N) {
            _bar("\"재귀함수는 자기 자신을 호출하는 함수라네\"\n", cnt, bw);
        } else {
            _bar("\"잘 들어보게. 옛날옛날 한 산 꼭대기에 이세상 모든 지식을 통달한 선인이 있었어.\n", cnt, bw);
            _bar("마을 사람들은 모두 그 선인에게 수많은 질문을 했고, 모두 지혜롭게 대답해 주었지.\n", cnt, bw);
            _bar("그의 답은 대부분 옳았다고 하네. 그런데 어느 날, 그 선인에게 한 선비가 찾아와서 물었어.\"\n", cnt, bw);
            solve(cnt + 1, bw);
        }
        _bar("라고 답변하였지.\n", cnt, bw);
    }

    public static void main(String[] args) throws IOException {
        BufferedReader br = new BufferedReader(new InputStreamReader(System.in));
        BufferedWriter bw = new BufferedWriter(new OutputStreamWriter(System.out));

        N = Integer.parseInt(br.readLine());
        bw.write("어느 한 컴퓨터공학과 학생이 유명한 교수님을 찾아가 물었다.\n");
        solve(0, bw);

        bw.flush();
        bw.close();
        br.close();
    }
}
